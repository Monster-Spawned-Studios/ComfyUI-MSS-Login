...
def clear_all_nsfw_tags():
	...
	print(
		f"[mss_login::NSFWGuard] Cleared NSFW tags from {cleared_count} images (out of {total_images} total images, {error_count} errors)"
	)
...
def fix_incorrectly_cached_tags():
	...
	if fixed_count > 0:
		print(
			f"[mss_login::NSFWGuard] Fixed {fixed_count} incorrectly cached images (cleared tags for rescan)"
		)
	else:
