...
def _set_nsfw_tag(path: str, is_nsfw: bool, score: float, label: str):
    ...
    try:
        ...
        # For JPEG: Use EXIF (Windows-readable)
            ...
            try:
                ...
                if not piexif_available:
                    ...
                    exif_bytes = (
                        img.getexif().tobytes() if hasattr(img.getexif(), "tobytes") else None
                    )
                ...
                print(f"[mss-login::NSFWGuard] Warning: Could not write EXIF to {path}: {e}")
        ...
        print(f"[mss-login::NSFWGuard] Warning: Could not write metadata to {path}: {e}")
...
def clear_nsfw_tag(path: str):
    ...
    try:
        ...
        print(f"[mss-login::NSFWGuard] Warning: Could not clear metadata from {path}: {e}")
...
def _get_nsfw_pipeline():
    ...
    if os.path.exists(os.path.join(local_model_dir, "config.json")):
        ...
        print(f"[mss-login::NSFWGuard] ⚠️ Local model missing. Downloading: {HF_MODEL_ID}")
    ...
    try:
        ...
        print(f"[mss-login::NSFWGuard] ❌ CRITICAL: Failed to load NSFW model. Error: {e}")
        return None
        ...
        if rec is not None:
            ...
            if not quiet:
                if _LAST_LOGGED_USER != cache_key:
                    print(f"[mss-login] 🛡️ Policy Check: User='{username}' | SFW={sfw_flag}")
                    _LAST_LOGGED_USER = cache_key
        else:
            # Cache the default
            _SFW_CACHE[cache_key] = (sfw_flag, username)
            if not quiet:
                if _LAST_LOGGED_USER != cache_key:
                    print(f"[mss-login] ⚠️ User '{username}' not found in DB. Defaulting to BLOCK.")
                    _LAST_LOGGED_USER = cache_key
    else:
...
def clear_all_nsfw_tags():
    ...
    print(f"[mss-login::NSFWGuard] Starting to clear NSFW tags from output directory: {output_dir}")
            ...
            for file in files:
                if file.lower().endswith((".png", ".jpg", ".jpeg")):
                    ...
                    try:
                        ...
                        error_count += 1
                        print(f"[mss-login::NSFWGuard] Error clearing tag from {path}: {e}")
    except Exception as e:
...
def fix_incorrectly_cached_tags():
    ...
    if fixed_count > 0:
        ...
        print("[mss-login::NSFWGuard] No incorrectly cached images found")
    return fixed_count
...
def scan_all_images_in_output_directory(force_rescan: bool = False):
    ...
    print(f"[mss-login::NSFWGuard] Starting batch scan of output directory: {output_dir}")
