    ...
    try:
        ...
        if ext in (".png",):
            ...
            if NSFW_METADATA_KEY in info:
                ...
                result = {"is_nsfw": is_nsfw, "score": score, "label": label}
                return result
            ...
            if "Keywords" in info:
                ...
                if "nsfw" in keywords:
                    ...
                    result = {"is_nsfw": True, "score": score, "label": label}
                    return result
        ...
        # For JPEG: Check EXIF (both our custom tag and Windows-readable fields)
            ...
            try:
                ...
                try:
                    ...
                    if (
                        "0th" in exif_dict
                        and piexif.ImageIFD.XPKeywords in exif_dict["0th"]
                    ):
                        ...
                        if "nsfw" in keywords.lower():
                            ...
                            if (
                                "0th" in exif_dict
                                and piexif.ImageIFD.XPComment in exif_dict["0th"]
                            ):
                                ...
                                if score_match:
                                    score = float(score_match.group(1))
                            return {"is_nsfw": True, "score": score, "label": label}
                except ImportError:
        ...
        if NSFW_METADATA_KEY in info:
            ...
            score = float(info.get(NSFW_SCORE_KEY, "0.0"))
            label = info.get(NSFW_LABEL_KEY, "")
            return {"is_nsfw": is_nsfw, "score": score, "label": label}
    ...
    try:
        ...
        if ext in (".png",):
            ...
            if is_nsfw:
                ...
                if existing_comment and "NSFW Content Detected" not in str(
                    existing_comment
                ):
                    # Preserve existing comment, append NSFW info
                    pnginfo.add_text(
                        "Comment",
                        f"{existing_comment} | NSFW Content Detected - Score: {score:.2f}, Label: {label}",
                    )
                else:
            ...
            try:
                ...
                if piexif_available:
                    ...
                    nsfw_data = json.dumps(
                        {"is_nsfw": is_nsfw, "score": score, "label": label}
                    )
                    exif_dict["Exif"][piexif.ExifIFD.UserComment] = (
                        f"NSFW:{nsfw_data}".encode("utf-8")
                    )
                    ...
                    if is_nsfw:
                        ...
                        if (
                            ...
                            existing_xpkeywords = exif_dict["0th"][
                                piexif.ImageIFD.XPKeywords
                            ]
                            if isinstance(existing_xpkeywords, bytes):
                                existing_xpkeywords = existing_xpkeywords.decode(
                                    "utf-16le", errors="ignore"
                                )
                        if (
                            existing_xpkeywords
                            and "nsfw" not in existing_xpkeywords.lower()
                        ):
                            # Preserve existing keywords and append NSFW
                            exif_dict["0th"][piexif.ImageIFD.XPKeywords] = (
                                f"{existing_xpkeywords}, NSFW".encode("utf-16le")
                            )
                        else:
                            exif_dict["0th"][piexif.ImageIFD.XPKeywords] = (
                                "NSFW".encode("utf-16le")
                            )
                        ...
                        if (
                            "0th" in exif_dict
                            and piexif.ImageIFD.XPSubject in exif_dict["0th"]
                        ):
                            existing_xpsubject = exif_dict["0th"][
                                piexif.ImageIFD.XPSubject
                            ]
                            if isinstance(existing_xpsubject, bytes):
                                existing_xpsubject = existing_xpsubject.decode(
                                    "utf-16le", errors="ignore"
                                )
                        if (
                            existing_xpsubject
                            and "NSFW Content" not in existing_xpsubject
                        ):
                            # Preserve existing subject, append NSFW info
                            exif_dict["0th"][piexif.ImageIFD.XPSubject] = (
                                f"{existing_xpsubject} | NSFW Content (Score: {score:.2f})".encode(
                                    "utf-16le"
                                )
                            )
                        else:
                            # No existing subject or it's already NSFW-related, set new one
                            exif_dict["0th"][piexif.ImageIFD.XPSubject] = (
                                f"NSFW Content (Score: {score:.2f})".encode("utf-16le")
                            )
                        ...
                        if (
                            ...
                            existing_xpcomment = exif_dict["0th"][
                                piexif.ImageIFD.XPComment
                            ]
                            if isinstance(existing_xpcomment, bytes):
                                existing_xpcomment = existing_xpcomment.decode(
                                    "utf-16le", errors="ignore"
                                )
                        if (
                            existing_xpcomment
                            and "NSFW Content Detected" not in existing_xpcomment
                        ):
                            # Preserve existing comment, append NSFW info
                            exif_dict["0th"][piexif.ImageIFD.XPComment] = (
                                f"{existing_xpcomment} | NSFW Content Detected - Score: {score:.2f}, Label: {label}".encode(
                                    "utf-16le"
                                )
                            )
                        else:
                            # No existing comment or it's already NSFW-related, set new one
                            exif_dict["0th"][piexif.ImageIFD.XPComment] = (
                                f"NSFW Content Detected - Score: {score:.2f}, Label: {label}".encode(
                                    "utf-16le"
                                )
                            )
                ...
                if not piexif_available:
                    ...
                    exif_bytes = (
                        img.getexif().tobytes()
                        if hasattr(img.getexif(), "tobytes")
                        else None
                    )
                ...
                print(
                    f"[mss-login::NSFWGuard] Warning: Could not write EXIF to {path}: {e}"
                )
        ...
        print(
            f"[mss-login::NSFWGuard] Warning: Could not write metadata to {path}: {e}"
        )


def set_nsfw_tag_manual(
    path: str, is_nsfw: bool, score: float = 1.0, label: str = "manual"
):
    """
...
def clear_nsfw_tag(path: str):
    ...
    try:
        ...
        if ext in (".png",):
            ...
            for key, value in img.info.items():
                ...
                if key == "Keywords":
                    ...
                    filtered_keywords = [
                        k for k in keywords_list if k.upper() not in ("NSFW", "SFW")
                    ]
        ...
        # For JPEG: Remove only NSFW-related metadata, preserve everything else
            ...
            try:
                ...
                if (
                    ...
                    user_comment = exif_dict["Exif"][piexif.ExifIFD.UserComment]
                    if isinstance(user_comment, bytes) and user_comment.startswith(
                        b"NSFW:"
                    ):
                ...
                if "0th" in exif_dict:
                    ...
                    if piexif.ImageIFD.XPKeywords in exif_dict["0th"]:
                        ...
                        if isinstance(keywords_bytes, bytes):
                            keywords_str = keywords_bytes.decode(
                                "utf-16le", errors="ignore"
                            )
                            keywords_list = [
                                k.strip() for k in keywords_str.split(",") if k.strip()
                            ...
                            filtered_keywords = [
                                ...
                                if k.upper() not in ("NSFW", "SFW")
                            ]
                            if filtered_keywords:
                                # Preserve cleaned keywords
                                exif_dict["0th"][piexif.ImageIFD.XPKeywords] = (
                                    ", ".join(filtered_keywords).encode("utf-16le")
                                )
                            else:
                    ...
                    if piexif.ImageIFD.XPSubject in exif_dict["0th"]:
                        ...
                        if isinstance(subject_bytes, bytes):
                            subject_str = subject_bytes.decode(
                                "utf-16le", errors="ignore"
                            )
                    ...
                    if piexif.ImageIFD.XPComment in exif_dict["0th"]:
                        ...
                        if isinstance(comment_bytes, bytes):
                            comment_str = comment_bytes.decode(
                                "utf-16le", errors="ignore"
                            )
                ...
                if hasattr(img, "getexif"):
                    ...
                    for tag_id, value in exif.items():
                        ...
                        if tag_id != 0x9286 or (
                            isinstance(value, str) and not value.startswith("NSFW:")
                        ):
        ...
        print(
            f"[mss-login::NSFWGuard] Warning: Could not clear metadata from {path}: {e}"
        )
...
def set_latest_prompt_user(username: str | None):
    ...
    print(f"[mss-login::NSFWGuard] set_latest_prompt_user → {effective!r}")
...
def _get_nsfw_pipeline():
    ...
    if os.path.exists(os.path.join(local_model_dir, "config.json")):
        ...
        # print(f"[mss-login::NSFWGuard] ✅ Loading local model from: {local_model_dir}")
    else:
        model_source = HF_MODEL_ID
        print(
            f"[mss-login::NSFWGuard] ⚠️ Local model missing. Downloading: {HF_MODEL_ID}"
        )
    ...
    try:
        ...
        print(
            f"[mss-login::NSFWGuard] ❌ CRITICAL: Failed to load NSFW model. Error: {e}"
        )
        return None


def _classify_image_path(
    path: str, use_cache: bool = True
    ...
    try:
        ...
        print(f"[mss-login::NSFWGuard] Error reading image {path}: {e}")
        return None
...
def _resolve_effective_username() -> str:
    ...
    print(
        f"[mss-login::NSFWGuard] DEBUG resolve_user: ctx={ctx_user!r} "
        ...
        if rec is not None:
            ...
            if not quiet:
                if _LAST_LOGGED_USER != cache_key:
                    print(
                        f"[mss-login] 🛡️ Policy Check: User='{username}' | SFW={sfw_flag}"
                    )
                    _LAST_LOGGED_USER = cache_key
        else:
            # Cache the default
            ...
            if not quiet:
                if _LAST_LOGGED_USER != cache_key:
                    print(
                        f"[mss-login] ⚠️ User '{username}' not found in DB. Defaulting to BLOCK."
                    )
                    _LAST_LOGGED_USER = cache_key
    else:
...
def should_block_image_for_current_user(
    path: str, quiet: bool = False, use_cache: bool = True
) -> bool:
    """
        ...
        if tag is not None:
            ...
            if cached_is_nsfw:
                if not quiet:
                    ...
                    print(
                        f"[mss-login::NSFWGuard] 🛑 BLOCKED NSFW file (cached): "
                ...
                try:
                    ...
                    username = (
                        current_username_var.get() or _LATEST_PROMPT_USER or "unknown"
                    )
                    send_notification(
                        "nsfw_block",
    ...
    if should_block:
        if not quiet:
            print(
                f"[mss-login::NSFWGuard] 🛑 BLOCKED NSFW file: "
...
def clear_all_nsfw_tags():
    ...
    print(
        f"[mss-login::NSFWGuard] Starting to clear NSFW tags from output directory: {output_dir}"
    )
            ...
            for file in files:
                if file.lower().endswith((".png", ".jpg", ".jpeg")):
                    ...
                    try:
                        ...
                        error_count += 1
                        print(
                            f"[mss-login::NSFWGuard] Error clearing tag from {path}: {e}"
                        )
    except Exception as e:
        print(f"[mss-login::NSFWGuard] Critical error in clear_all_nsfw_tags: {e}")

    print(
        f"[mss-login::NSFWGuard] Cleared NSFW tags from {cleared_count} images (out of {total_images} total images, {error_count} errors)"
    )
...
def fix_incorrectly_cached_tags():
    ...
    if fixed_count > 0:
        print(
            f"[mss-login::NSFWGuard] Fixed {fixed_count} incorrectly cached images (cleared tags for rescan)"
        )
    else:
        print(f"[mss-login::NSFWGuard] No incorrectly cached images found")
    return fixed_count
...
def scan_all_images_in_output_directory(force_rescan: bool = False):
    ...
    print(
        f"[mss-login::NSFWGuard] Starting batch scan of output directory: {output_dir}"
    )
        ...
        for file in files:
            if file.lower().endswith((".png", ".jpg", ".jpeg")):
                ...
                try:
                    ...
                    error_count += 1
                    print(f"[mss-login::NSFWGuard] Error scanning {path}: {e}")
    ...
    print(f"[mss-login::NSFWGuard] Batch scan complete: {result}")
    return result
