...
def install_dependencies() -> bool:
    ...
    try:
        ...
        if not run_command(["dotenvx", "--version"]):
            print("[mss_login] Dotenvx binary not found. Please install it manually using `pip install python-dotenvx`.", file=sys.stderr)
            print("[mss_login] Running `dotenvx-postinstall` to install the dotenvx binary...", file=sys.stderr)
            if not run_command(["dotenvx-postinstall"]):
                print("[mss_login] Failed to install dotenvx binary. Please install it manually using `dotenvx-postinstall`.", file=sys.stderr)
                return False
            else:
