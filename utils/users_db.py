    ...
    def migrate_from_json(self, legacy_path: str) -> bool:
        ...
        for uid, u in items:
            ...
            self._conn.execute(
                ...
                """,
                (uid, username, password, 1 if admin else 0, _groups_to_json(groups), 1 if sfw else 0),
            )
        self._conn.commit()
        try:
...
class UsersDB:
    ...
    def update_user(self, username: str, groups: list, is_admin: bool, sfw_check: Optional[bool] = None) -> bool:
    ...
    def delete_user(self, username: str) -> bool | str:
        ...
        if not user_id:
            return False
        admins = sum(1 for u in self.users.values() if u.get("admin") or "admin" in [g.lower() for g in u.get("groups", [])])
        if (user.get("admin") or "admin" in [g.lower() for g in user.get("groups", [])]) and admins <= 1:
            return "last_admin"
