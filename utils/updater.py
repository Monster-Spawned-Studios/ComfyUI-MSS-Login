...
_config_for_updater = _read_config_json(
    join(_REPO_ROOT, "config.json")
) or _read_config_json(join(_REPO_ROOT, "config.defaults.json"))
...
def _parse_version(s: str) -> Any:
    ...
    try:
        ...
        return (
            tuple(int(x) for x in num_str.split(".") if x.isdigit())
            if num_str
            else (0, 0, 0)
        )
...
async def check_for_update(
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            for item in data:
                name = (
                    (item.get("name") or item.get("tag_name") or "").lstrip("v").strip()
                )
...
def perform_update(
    ...
    if not backup_path and os.path.isdir(os.path.join(data_dir, "data")):
        logger.warning(
            "[mss-login] Update backup skipped (backup dir creation failed)."
        )
    try:
        ...
        if os.path.isfile(req):
            try:
                ...
                if DEBUG_MODE:
                    print(
                        f"[mss-login] Auto-update: installed dependencies from requirements.txt"
                    )
                subprocess.run(
                ...
                if DEBUG_MODE:
                    print(
                        f"[mss-login] Auto-update: installed dependencies from pyproject.toml"
                    )
            except Exception:
                print(
