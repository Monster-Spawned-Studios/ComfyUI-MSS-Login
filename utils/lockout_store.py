...
class _SqliteLockoutStore:
    ...
    def get_blacklist_with_expiry(self) -> list[tuple[str, Optional[int]]]:
        ...
        rows = self._conn.execute(
            f"SELECT ip, expires_at FROM {TABLE_IP}"
        ).fetchall()
    ...
    def remove_whitelist_entry(self, entry: str) -> bool:
        cur = self._conn.execute(
            f"DELETE FROM {TABLE_WHITELIST} WHERE entry = ?", (entry,)
        )
        self._conn.commit()
        return cur.rowcount > 0

    def add_blacklist_entry(
        self, ip: str, expires_at: Optional[int] = None
    ) -> None:
        try:
            self._conn.execute(
                f"INSERT OR REPLACE INTO {TABLE_IP} (ip, expires_at) VALUES (?, ?)",
    ...
    def set_whitelist(self, entries: list[str]) -> None:
        try:
            ...
            for entry in entries:
                ...
                if entry:
                    cur.execute(
                        "INSERT IGNORE INTO {} (`entry`) VALUES (%s)".format(
                            TABLE_WHITELIST
                        ),
                        (entry,),
                    )
    ...
    def set_blacklist(self, entries: list[tuple[str, Optional[int]]]) -> None:
        try:
            ...
            for ip, expires_at in entries:
                ...
                if ip:
                    cur.execute(
                        "INSERT INTO {} (ip, expires_at) VALUES (%s, %s)".format(
                            TABLE_IP
                        ),
                        (ip, expires_at),
                    )
...
_store: Optional[
    _SqliteLockoutStore | _PostgresLockoutStore | _MySQLLockoutStore
] = None
