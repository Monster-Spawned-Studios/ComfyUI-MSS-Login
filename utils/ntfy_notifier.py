...
def save_ntfy_config(
    ...
    cfg["ntfy"] = {
        ...
        "enabled_events": (
            list(enabled_events) if isinstance(enabled_events, list) else []
        ),
...
def notify_user_created(
    ...
    **kwargs,
    ...
    return send_notification(
        ...
        message=(
            f"New user: **{new_username}** "
            f"(registered by {registered_by}) from IP: `{ip}`"
        ),
        markdown=True,
        **kwargs,
    )
...
def notify_user_role_changed(
    ...
    **kwargs,
    ...
    return send_notification(
        ...
        message=(
            f"User **{username}** role changed from "
            f"`{old_role}` to `{new_role}`{actor}."
        ),
        markdown=True,
        **kwargs,
    )
