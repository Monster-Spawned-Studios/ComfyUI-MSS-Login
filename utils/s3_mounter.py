...
def _sanitize_username(username: str) -> str:
	safe = "".join(c if c.isalnum() or c in ("_", "-") else "_" for c in (username or "").strip())
...
def _load_runtime_config() -> dict:
	...
	return {
		...
		"access_key_id": (
			os.getenv(access_env) or _load_encrypted_setting(_ACCESS_KEY_SETTING)
		).strip(),
		"secret_access_key": (
			os.getenv(secret_env) or _load_encrypted_setting(_SECRET_KEY_SETTING)
		).strip(),
		...
		"mount": {
			...
			"model_folders": [
				str(folder).strip() for folder in model_folders if str(folder).strip()
			],
		...
		"workflow_sync": {
			...
			"conflict_strategy": (workflow.get("conflict_strategy") or "newer_wins")
			.strip()
			.lower(),
...
def save_s3_settings(payload: dict) -> dict:
	...
	workflow["sync_interval_seconds"] = _safe_int(payload.get("workflow_sync_interval_seconds"), 60)
	workflow["conflict_strategy"] = (
		(payload.get("workflow_conflict_strategy") or "newer_wins").strip().lower()
	)
...
class S3MountManager:
	...
	def _write_passwd_file(self) -> None:
		...
		with open(self._passwd_path, "w", encoding="utf-8") as handle:
			handle.write(f"{self._cfg['access_key_id']}:{self._cfg['secret_access_key']}")
...
def init_mount_manager(
	s3_config: dict | None = None, mount_config: dict | None = None, data_dir: str | None = None
) -> S3MountManager:
