...
class Logger:
    ...
    def _maybe_rotate(self) -> None:
        ...
        now = time.time()
        interval_sec = (
            (self._rotation_interval_hours or 0) * 3600
        )
        size_ok = (
            self._rotation_max_bytes is None
            or size < self._rotation_max_bytes
        )
        time_ok = (
            interval_sec <= 0
            or (now - self._last_rotate_time) < interval_sec
        )
    ...
    def login_attempt(self, ip: str, username: str) -> None:
        ...
        self._log_console(
            "WARNING", f"Attempted login for username: '{username}' from IP: {ip}"
        )
    ...
    def login_failed(self, ip: str, username: str) -> None:
        ...
        self._log_console(
            "WARNING", f"Failed login for username: '{username}' from IP: {ip}"
        )

    def generate_attempt(
        self, ip: str, username: str, expire_hours: int
    ) -> None:
    ...
    def registration_success(
        ...
        if registered_by:
            self._log_file(
                "INFO", f"New user: '{new_user}' registered by '{registered_by}'"
            )
            self._log_console(
                "INFO",
