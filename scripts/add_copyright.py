...
def wrap_comment(copyright_text: str, style_key: str) -> str:
    ...
    lines = [line.strip()
             for line in copyright_text.splitlines() if line.strip()]
    if kind == "line":
...
def main() -> int:
    ...
    if not copyright_path.is_file():
        print(
            f"Error: COPYRIGHT file not found: {copyright_path}", file=sys.stderr)
        return 1
    ...
    if copyright_text != raw_copyright:
        if args.dry_run:
            print(
                f"Dry run: would update year in {copyright_path.relative_to(root)}")
        else:
            copyright_path.write_text(copyright_text + "\n", encoding="utf-8")
            print(f"Updated year in {copyright_path.relative_to(root)}")
    copyright_lines = [s.strip()
                       for s in copyright_text.splitlines() if s.strip()]
