...
def get_current_year() -> int:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            if raw:
                # Parse RFC 2822 date (e.g. "Fri, 13 Feb 2026 12:00:00 GMT")
...
def already_has_copyright(content: str, copyright_lines: list[str]) -> bool:
    ...
    return first_line in head and (
        "monsterspawned" in head.lower() or "Monster Spawned" in head or "All Rights Reserved" in head
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Path to COPYRIGHT file (default: docs/COPYRIGHT under root)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
