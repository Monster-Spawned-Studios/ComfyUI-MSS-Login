...
if not exists(f"{getcwd()}/logs"):
    try:
        ...
        logger.error(
            "Please create the logs directory manually and try again.")
        logger.error("The logs directory should be located at: %s",
                     f"{getcwd()}/logs")
...
def setup_dev(
    ...
    try:
        subprocess.run(["uv", "venv", "--python",
                       f"{python_version}"], check=True)
    except subprocess.CalledProcessError as e:
        logger.error(
            "Failed to create a virtual environment for the project: %s", e)
        return False
    logger.debug(
    ...
    try:
        ...
        logger.error(
            "Failed to install the dependencies for the project: %s", e)
        return False
    logger.debug(
    ...
    try:
        subprocess.run(["comfy", "install", "--group",
                       comfyui_group], check=True)
    except subprocess.CalledProcessError as e:
...
def install_comfyui(
    ...
    try:
        if (system() == "Darwin" and machine() == "arm64") or arg_values.get(
            ...
            logger.error(
                "No supported platform found for ComfyUI installation.")
            return False
    except subprocess.CalledProcessError as e:
        logger.error(
    ...
    try:
        try:
            result = subprocess.run(
                ["uv", "--help"], check=True, capture_output=True, text=True)
            if result.returncode != 0:
                logger.error(
                    "Failed to check the uv version: %s", result.stderr)
                sleep(3)
                exit(1)
        except subprocess.CalledProcessError as e:
            logger.error(
                "Failed to check the uv version: %s\nPlease install uv manually using the `pip install -U uv` command.",
                e,
            )
            sleep(3)
            exit(1)
        if setup_dev(
            ...
            if arg_values.get("install_comfyui", False):
                if install_comfyui(
                    ...
                    logger.info(
                        "ComfyUI and its dependencies installed successfully.")
                else:
                    logger.error(
                        "Failed to install ComfyUI and its dependencies.")
            else:
                logger.info(
        ...
        logger.error(
            "Failed to setup the local development environment: %s", e)
