...
### 🎨 Extension Tabs API
- **New API for extensions** - Extensions can now register custom tabs in the mss_login admin panel
- **Tab registry system** - Global `window.mss_loginAdminTabs` API for tab registration and management
...
- **PUT endpoint implementation** - Completed the `/mss_login/api/ip-lists` PUT endpoint for saving IP rules
...
- **Guest logout visibility** - Ensured logout button and mss_login menu are always visible for guest accounts
...
- **Gallery integration endpoint** - New `/mss_login-gallery/mark-nsfw` endpoint for manual image flagging from gallery UIs
...
### 🔗 Gallery Integration
- **ComfyUI-mss_login-Gallery compatibility** - Full integration with gallery extension
