...
| Auth | GET | `/mss-login/api/tokens` | `get_user_tokens` | Return the calling user's API tokens (labels, expiry, hash prefix). Session token required. |
...
| Admin | GET | `/mss-login/api/settings/model-isolation-download-patterns` | `api_get_model_isolation_download_patterns` | Return configured/effective model download redirect patterns (owner only). |
| Admin | PUT | `/mss-login/api/settings/model-isolation-download-patterns` | `api_put_model_isolation_download_patterns` | Update owner-defined model download redirect patterns. |
...
| Admin | GET | `/mss-login/api/users/{username}/shared-items` | `api_get_shared_items` | List shared model grants for a user (owner/admin with sharing permission). |
| Admin | POST | `/mss-login/api/users/{username}/shared-items` | `api_add_shared_item` | Add/update one shared item for a user. Body supports folder, item_name, source_backend. |
...
| MFA | POST | `/mss-login/api/mfa/setup` | `api_mfa_setup` | Start MFA setup. Requires JWT (logged-in user) or mfa_temp_token (from login when MFA is pending). |
