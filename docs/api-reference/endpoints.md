...
| Auth | GET | `/mss-login/api/tokens` | `get_user_tokens` | Return the calling user's API tokens (labels, expiry, hash prefix). Session token |
...
| MFA | POST | `/mss-login/api/mfa/setup` | `api_mfa_setup` | Start MFA setup. Requires JWT (logged-in user) or mfa_temp_token (from login when |
