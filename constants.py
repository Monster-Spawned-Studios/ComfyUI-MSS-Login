...
if TOKEN_ALGORITHM not in ("HS256", "RS256", "ES256", "PS256"):
	warnings.warn(f"[MSS-Login] Invalid token algorithm: {TOKEN_ALGORITHM}. Using HS256.")
...
def _get_recovery_mode() -> bool:
	env_val = (
		str(os.environ.get("RECOVERY_MODE", "") or os.environ.get("RECOVERY_MODE_ENABLED", ""))
		.strip()
		.lower()
	)
...
def _get_recovery_mode_hosts() -> list:
	...
	raw = (
		os.environ.get("RECOVERY_MODE_HOST") or os.environ.get("RECOVERY_MODE_HOSTS") or ""
	).strip()
	if not raw:
		return ["127.0.0.1", "::1"]
