...
WHITELIST_FILE = _resolve_data_path(
    config_data.get("whitelist", "data/whitelist.txt"))
BLACKLIST_FILE = _resolve_data_path(
    config_data.get("blacklist", "data/blacklist.txt"))
...
LOG_ROTATION_MAX_BYTES = int(_log_rot_cfg.get(
...
LOG_ROTATION_ARCHIVE_DIR = (
    _resolve_data_path(
        _log_archive) if _log_archive else os.path.dirname(LOG_FILE)
)
...
EPHEMERAL_SECRET_KEY_PATH = os.path.join(
    DATA_DIR, "data", ".ephemeral_secret_key")
...
TOKEN_EXPIRE_MINUTES = 60 * \
    config_data.get("access_token_expiration_hours", 12)
MAX_TOKEN_EXPIRE_MINUTES = 60 * \
    config_data.get("max_access_token_expiration_hours", 8760)
...
_users_sqlite_path = _env_or_config(
    "USERS_DB_SQLITE_PATH", _users_db_cfg.get(
...
USERS_DB_CONFIG = {
    ...
    "postgres_host": _env_or_config(
        "USERS_DB_POSTGRES_HOST", _users_db_cfg.get(
            "postgres_host", "localhost")
    ),
    "postgres_port": _env_or_config(
        "USERS_DB_POSTGRES_PORT", str(
            _users_db_cfg.get("postgres_port", 5432))
    ),
    ...
    "postgres_user": _env_or_config(
        "USERS_DB_POSTGRES_USER", _users_db_cfg.get(
            "postgres_user", "mss_login")
    ),
...
def get_host_base_url(use_https: bool = True, use_port: bool = False, port: int = 8188) -> str:
    """
...
def reload_users_db_config() -> dict:
    ...
    USERS_DB_CONFIG = {
        ...
        "mysql_host": _env_or_config(
            "USERS_DB_MYSQL_HOST", _users_db_cfg.get(
                "mysql_host", "localhost")
        ),
        "mysql_port": _env_or_config(
            "USERS_DB_MYSQL_PORT", str(
                _users_db_cfg.get("mysql_port", 3306))
        ),
        "mysql_database": _env_or_config(
            "USERS_DB_MYSQL_DATABASE",
            _users_db_cfg.get("mysql_database", "mss_login"),
        ...
        "mysql_user": _env_or_config(
            "USERS_DB_MYSQL_USER", _users_db_cfg.get(
                "mysql_user", "mss_login")
        ),
        "encryption_level": _normalize_encryption_level(_users_db_cfg.get("encryption_level", "")),
    }
    USERS_DB_CONFIG["postgres_password"] = (
...
try:
    SESSION_IDLE_REVOKE_MINUTES = int(
        config_data.get("session_idle_revoke_minutes", 5))
except (TypeError, ValueError):
    SESSION_IDLE_REVOKE_MINUTES = 5

# Remote API guard: require auth for non-local clients
REQUIRE_AUTH_FOR_REMOTE_API = config_data.get(
    "require_auth_for_remote_api", True)
...
def _get_recovery_mode() -> bool:
    env_val = (
        str(os.environ.get("RECOVERY_MODE", "")
...
def _get_recovery_mode_hosts() -> list:
    ...
    raw = (
        os.environ.get("RECOVERY_MODE_HOST") or os.environ.get(
            "RECOVERY_MODE_HOSTS") or ""
    ).strip()
    if not raw:
        return ["127.0.0.1", "::1"]
