...
if USERS_DB_CONFIG.get("backend") == "sqlite" and not USERS_DB_CONFIG.get(
    "encryption_level"
):
    ...
    if _old_token_path and not os.path.isabs(_old_token_path):
        _old_token_path = os.path.join(
            os.path.dirname(CONFIG_FILE_PATH), _old_token_path
        )
    ...
    if _migrated:
        for _path in (WHITELIST_FILE, BLACKLIST_FILE):
            try:
                ...
                pass
        logger.info(
            "[mss_login] Migrated IP whitelist/blacklist from files to database."
        )
