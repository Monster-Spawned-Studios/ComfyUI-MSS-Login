...
def _local_path_allowed(local_path: str) -> bool:
    ...
    try:
        ...
        pass
    return any(
        base and (resolved == base or resolved.startswith(base + os.sep))
        for base in bases
    )
...
async def s3_delete(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"message": "Object deleted." if deleted else "Object not found.", "deleted": deleted})
    except Exception as exc:
        return _error_json(str(exc), 500)
...
async def s3_mount_unmount(request: web.Request) -> web.Response:
    ...
    if error:
        return error
    return web.json_response({"status": "ok", "message": mgr.unmount(), "mount_status": mgr.status()})
