...
async def post_register(request: web.Request) -> web.Response:
    ...
    logger.registration_success(
        ip, new_username, username if not is_first_admin else None
    )
    try:
        send_notification(
            "user_created",
...
async def post_generate_token(request: web.Request) -> web.Response:
    ...
    try:
        expire_hours = (
            float(expire_hours_raw) if expire_hours_raw not in (None, "") else 720.0
        )
    ...
    if token_from_request:
        try:
            ...
            if jwt_username and users_db.get_user(jwt_username)[0]:
                ...
                if expire_hours <= 0 and not _user_can_have_non_expiring_jwt(
                    jwt_username
                ):
                    return web.json_response(
                        {
                ...
                user_console_append(
                    jwt_username, f"API token created for user: {jwt_username}"
                )
                try:
                    send_notification(
                        "api_token_created",
    ...
    if mfa_temp and (mfa_code or mfa_backup):
        ...
        if mfa_backup:
            ...
            return web.json_response(
                {"error": "Provide code or backup_code"}, status=400
            )
        ...
        if DEBUG_MODE:
            ...
            logger.log_jwt_created_console_only(username)
        user_console_append(
            username, f"API token created for user: {username} (after MFA)"
        )
        try:
            send_notification(
                "api_token_created",
            ...
            pass
        logger.generate_success(
            ip, username, int(expire_hours) if expire_hours > 0 else 0
        )
        return web.json_response(
            {
                "message": "API token created.",
