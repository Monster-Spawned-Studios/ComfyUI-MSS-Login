				...
				json.dumps(
					{
						...
						"data": {"path": path, "is_admin": is_admin, "username": username},
						"hypothesisId": "A",
...
# Frontend calls /mss_login/api/me (underscore); ComfyUI may send /api/mss_login/api/me. Register aliases so both match.
routes.get("/mss_login/api/me")(api_me)
routes.get("/api/mss_login/api/me")(api_me)
...
async def api_user_env(request: web.Request) -> web.Response:
	...
	if action == "delete_file":
		...
		if os.path.exists(full) and os.path.isfile(full):
			try:
				...
				return web.json_response({"user": target_user, "file": rel, "message": msg})
			except Exception as e:
		...
		return web.json_response({"user": target_user, "file": rel, "message": msg}, status=404)
	...
	if action == "set_gallery_root":
		...
		print(f"[mss_login] {msg}")
		return web.json_response({"user": target_user, "message": msg, "is_gallery_root": is_root})
	...
	if action == "promote_workflow":
		...
		try:
			...
			return web.json_response({"error": f"Failed to promote workflow: {e}"}, status=500)
