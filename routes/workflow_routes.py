...
def user_is_admin(username: str) -> bool:
	...
	try:
		...
		if not user_obj:
			print(
				f"[mss_login] user_is_admin: unexpected record type for {username!r}: {type(record)} -> {record!r}"
			)
			return False
			...
			if os.path.isfile(img_path):
				try:
					if should_block_image_for_current_user(img_path):
						...
						print(
							f"[mss_login::NSFWGuard] Blocking NSFW image for user={username!r}: {img_path}"
						)
