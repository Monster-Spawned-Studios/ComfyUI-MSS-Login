...
def user_is_admin(username: str) -> bool:
    ...
    try:
        ...
        print(f"[mss-login] user_is_admin: {username!r} groups={groups!r} is_admin={is_admin}")
        return is_admin
...
async def delete_workflow(request, name: str | None):
    ...
    print(f"[mss-login] delete_workflow user={user!r}, is_admin={is_admin}, name={clean_name!r}")
        ...
        if filename and img_type == "output":
            ...
            if img_path and os.path.isfile(img_path):
                try:
                    ...
                    print(f"[mss-login::NSFWGuard] Error while checking {img_path}: {e}")
