...
def _resolve_username_from_token_and_data(request: web.Request, data: dict) -> tuple[str | None, str | None]:
...
async def api_mfa_setup(request: web.Request) -> web.Response:
    ...
    if err or not username:
        return web.json_response({"error": err or "Authentication required"}, status=401)
    result = users_db.mfa_setup_start(username)
    if not result:
        return web.json_response({"error": "MFA setup failed or user not found"}, status=400)
...
async def api_mfa_verify_setup(request: web.Request) -> web.Response:
    ...
    if not code or len(code) != 6:
        return web.json_response({"error": "Invalid code (must be 6 digits)"}, status=400)
...
async def api_mfa_verify(request: web.Request) -> web.Response:
    ...
    mfa_temp = (data.get("mfa_temp_token") or "").strip()
    code = (data.get("code") or "").strip().replace(" ", "")
    backup_code_raw = (data.get("backup_code") or "").strip().replace(" ", "").replace("-", "").upper()
    if not mfa_temp:
        return web.json_response({"error": "mfa_temp_token required"}, status=400)
    username = consume_mfa_temp_token(mfa_temp)
    if not username:
        return web.json_response({"error": "Invalid or expired token. Please log in again."}, status=401)
    if backup_code_raw:
    ...
    token = jwt_auth.create_access_token({"id": user_id, "username": username}, no_expiration=no_exp)
    try:
        payload = jwt_auth.decode_access_token(token)
        jti = payload.get("jti")
        exp = payload.get("exp")
        ...
        if jti:
            get_session_token_store(SESSION_TOKEN_STORE_PATH).register_session(jti, user_id, username, exp_at_iso)
    except Exception:
        pass
    if DEBUG_MODE:
