...
@routes.post("/api/mss_login/recovery/reset-mfa")
async def post_recovery_reset_mfa(request: web.Request) -> web.Response:
    ...
    if not _client_ip_allowed_for_recovery(client_ip):
        logger.error(f"[mss_login] Recovery reset-mfa rejected: client IP {client_ip!r} not in allowed hosts.")
        return web.json_response(
    ...
    try:
        ...
        logger.info(f"[mss_login] Recovery: MFA reset for {count} user(s) from IP {client_ip!r}.")
        return web.json_response({"message": f"MFA reset for {count} user(s). Users may log in with password and re-enroll MFA."})
    except Exception as e:
        logger.error(f"[mss_login] Recovery reset-mfa failed: {e}")
