...
async def workflow_interceptor_middleware(request, handler):
	...
	if path in ("/prompt", "/api/prompt") and method in ("POST", "PUT"):
		...
		if prompt_to_validate is not None:
			...
			if not can_view_all:
				user_id, _ = access_control.users_db.get_user(username=username_for_perm) if username_for_perm else (None, {})
				allowed_set = set()
				if user_id:
					store = get_shared_items_store(USERS_DB_CONFIG)
					allowed_set = store.get_all_for_user(user_id)
				valid, err_msg = validate_prompt_models(allowed_set, allow_all=False, prompt=prompt_to_validate)
				if not valid:
...
app.middlewares.append(
	jwt_auth.create_jwt_middleware(
		...
		public_prefixes=("/mss-login", "/mss-login-gallery", "/mss_login/api/mfa", "/assets", "/static"),
	)
)
