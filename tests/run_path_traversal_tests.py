...
def run_tests():
    ...
    with tempfile.TemporaryDirectory() as base:
        ...
        ok(resolved == os.path.normpath(f) or os.path.samefile(resolved, f), "resolved equals expected")
    with tempfile.TemporaryDirectory() as base:
    ...
    ok(resolve_path_under("/nonexistent/base/dir", "file.txt") is None, "nonexistent base returns None")
    with tempfile.TemporaryDirectory() as base:
        ...
        ok(resolve_path_under(base, None) is not None and resolve_path_under(base, None) == os.path.realpath(base), "None rel_path returns base")
